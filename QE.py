            ...
            if tokens[3] in Classes.keys():
                if tokens[QEcols['AGEGROUP']] in ageGroups:
                    if tokens[QEcols['FLEET']] in fleets:
                        ...
                        message = "ERROR: " + line + " unknown fleet " + tokens[FLEET]
                else:
                    message = "ERROR: " + line + " unknown age group"
            else:
                message = "ERROR: " + line +  "unknown class"
        elif len(tokens) != 1:
            message = "ERROR: line should have 7 tokens QE,HELM,CREW,CLASS,SAILNO,PERSONAL,AGERGROUP, Contains"
