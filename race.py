...
<th>Laps</th><th>Time</th><th>Corrected</th><th>Rank</th></tr>"
	...
	def summary(self):
		...
		for e in self.entries:
			results += str(e.QE.QE).ljust(5," ") + str(e.QE.helm).ljust(20," ") + str(e.QE.dinghy).ljust(10," ") + str(e.time).ljust(6," ") + str(e.laps).ljust(6," ") + str(e.correctedPY).ljust(10," ") + str(e.PYplace) + "\n"
		return results
	...
	def score(self, dnc):
		...
		self.maxLaps = 0
		for e in self.entries:
			if e.laps > self.maxLaps: self.maxLaps = e.laps
		...
		scored = 0
		for e in self.entries:
			if e.QE.personal == 0:e.personalPlace = dnc
		...
		while scored < len(self.entries):
			...
			for e in self.entries:
				if e.correctedPersonal == t: inplace  += 1
			for e in self.entries:
	...
	def save(self,f):
		with open(f,"w") as out:
			out.write(f +"\n")
			for e in self.entries:
				out.write(e.QE.QE + "," + e.QE.helm + "," + e.QE.crew + "," + e.QE.dinghy + "," +  str(e.QE.sailno) + "," + str(e.time) + "," + str(e.laps) + "," + e.finCode + ",\n")
			out.write("<HANDICAPS>\n")
			for h in self.handicaps: out.write(h+"\n")
			out.write("<QES>\n")
			for qe in self.QEs:
				out.write(qe.QE + "," + qe.helm + "," + qe.crew + "," + qe.dinghy + "," + str(qe.sailno) \
						  + "," + str(qe.personal) + "," + str(qe.ageGroup) + "," + str(qe.fleet) + "\n")
	...
	while line != "":
		...
		if len(tokens) == 1:		
			if tokens[0].upper() == "HELP":
				...
				print("CLASS          (lists all the boats matching the inputed class or part inputed class)")
				print("SAIL           (lists all boats matching the sail number or part sail number)")
