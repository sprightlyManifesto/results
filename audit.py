...
pth = os.getcwd()
files = []
subDirs = ['AM','PM','Blackaby','Houghton','Icicle','watts','caulcott','Wendensday']
for sd in subDirs:
	for f in os.listdir(path.join(pth,sd)): 		
		files.append(path.join(path.join(pth,sd),f))
...
f = os.path.join(os.getcwd() + "/audits/",datetime.datetime.now().strftime("%d %m %Y") + " audit.txt")
open(f,"w").write(csv)
f = os.path.join(os.getcwd() + "/audits/audit.txt")
open(f,"w").write(csv)
open("personalHandicaps.html", "w").write(html)


open("up.sh", "w").write('''sshpass -f ~/pass.txt scp Autumn.html stewartby-watersports.org.uk@web9.extendcp.co.uk:~/public_html/sailing/Results/2019/
sshpass -f ~/pass.txt scp personalHandicaps.html stewartby-watersports.org.uk@web9.extendcp.co.uk:~/public_html/sailing/Results/2019/
sshpass -f ~/pass.txt scp ''' + os.getcwd()+ "/audits/audit.txt stewartby-watersports.org.uk@web9.extendcp.co.uk:~/public_html/sailing/Results/2019/")
