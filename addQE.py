...
def setReadMode():
    ...
    for c in recordControls: c.config(state="disabled")
...
def setEditMode():
    ...
    for c in recordControls: c.config(state="normal")
    line = GUItoLine()
    print(line)
...
def submitButtonCallBack():
    ...
    if message == "":
        ...
        records.sort(key=lambda x:x.split(",")[1])
        for i, record in enumerate(records): Lb1.insert(i, record)
...
DinghyClassEntry    = Combobox(col2Frame,width=20,textvariable = dinghy,values=list(classes.keys()))
...
GrandMastersRadio   = Radiobutton(AgeGroupFrame,    text="Grandmaster"  ,variable=ageGroup,value="G")
GoldRadio           = Radiobutton(GoldSilverFrame,  text="Gold"         ,variable=goldSilverGroup,value="G")
SilverRadio         = Radiobutton(GoldSilverFrame,  text="Silver"       ,variable=goldSilverGroup,value="S")

recordControls = [QEcodeEntry,HelmEntry,CrewEntry,SailNoEntry,PersonalEntry,DinghyClassEntry,
                  JuniorRadio,SeniorRadio,MastersRadio,GrandMastersRadio,
                  GoldRadio, SilverRadio]

editButton          = Button(ButtonFrame,text="Edit",   command=setEditMode)
submitButton        = Button(ButtonFrame,text="Submit", command=submitButtonCallBack,state="disabled")
cancelButton        = Button(ButtonFrame,text="Cancel", command=cancelButtonCallBack,state="disabled")
...
#Layout
Label(top,text = f"handicaps: {handicapsFile}  Qes: {recordsFile}").pack(fill="x",side="top")
messageLabel.pack()
...
for l,e in [("Class", DinghyClassEntry),("SailNo",SailNoEntry),("Personal", PersonalEntry)]:
    Label(col2Frame,text=l).pack(side="top",anchor="w")
    e.pack(side="top",anchor="w")

topFrame.pack(side="top",fill="x", pady=10)   

for f in (JuniorRadio, SeniorRadio, MastersRadio, GrandMastersRadio, GoldRadio, SilverRadio):
    f.pack(padx=5)
